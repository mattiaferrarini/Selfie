  ...
  <div class="flex flex-col items-center justify-center min-h-screen p-3 bg-tropical">
    <div class="animate-fade-in w-11/12 p-2 sm:p-5 rounded shadow-2xl shadow-emerald-600 bg-white">
      <fieldset class="border-2 rounded-xl border-emerald-500 p-2 sm:p-4">
        ...
        <input type="password" v-on:keyup.enter="changePassword" v-model="old_password" id="old_password" class="w-full px-3 py-2 mb-3 border rounded"
               placeholder="Old Password"/>
        <label for="new_password" class="block text-sm font-medium text-gray-700">Nuova Password</label>
        <input type="password" v-on:keyup.enter="changePassword" v-model="new_password" id="new_password" class="w-full px-3 py-2 mb-3 border rounded"
               placeholder="New Password"/>
        <label for="new_password_r" class="block text-sm font-medium text-gray-700">Ripeti Nuova Password</label>
        <input type="password" v-on:keyup.enter="changePassword" v-model="new_password_r" id="new_password_r" class="w-full px-3 py-2 mb-3 border rounded"
               placeholder="Repeat New Password"/>
        <button @click="changePassword" v-on:keyup.enter="changePassword" class="w-full px-3 py-2 text-white bg-emerald-500 rounded">Cambia Password
        </button>
